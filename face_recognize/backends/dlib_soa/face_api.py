...
def to_rect(face_info):
    return dlib.rectangle(face_info.top, face_info.right, face_info.bottom, face_info.left)
...
def init_engine(mode='image', num_face=10, mask='all'):
    if mask == 'recognize':
        ...
        return pose_predictor_68_point, face_encoder
    else:
...
def detect(engine, img, mode='image',number_of_times_to_upsample=1):
    ...
    for location in locations:
        face_infos.append(FaceInfo(0, max(location.left(), 0),
                                   max(location.top(), 0), min(location.right(), img.shape[1]),
                                   min(location.bottom(), img.shape[0]), 0))
    return face_infos
